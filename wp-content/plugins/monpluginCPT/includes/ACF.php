...
public function save( $post_id ) {
    ...
    if ( ! isset( $_POST['_apprenant_' .$this->meta_name. '_nonce'] ) ) {
        return $post_id;
    }

    $nonce = $_POST['_apprenant_' .$this->meta_name. '_nonce'];

    // Vérifier que le nonce est valide.
    if ( ! wp_verify_nonce( $nonce, '_apprenant_' .$this->meta_name ) ) {
        return $post_id;
    }
    ...
    $mydata = sanitize_text_field( $_POST['_apprenant_' .$this->meta_name] );
...
// Ajoutez un champ nonce pour que nous puissions le vérifier plus tard.
    wp_nonce_field( '_apprenant_'.$this->meta_name, '_apprenant_'.$this->meta_name.'_nonce' );
    ...
    ?>
    <label for="_apprenant_<?php echo $this->meta_name; ?>">
        <?php _e($this->name_display, 'default' ); ?>
    </label>
    <input type="text" id="_apprenant_<?php echo $this->meta_name; ?>" name="_apprenant_<?php echo $this->meta_name; ?>" value="<?php echo esc_attr( $value ); ?>" size="25" />
    <?php
}
}